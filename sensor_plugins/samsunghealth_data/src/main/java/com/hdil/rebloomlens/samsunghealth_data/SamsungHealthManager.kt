        ...
        for (i in 0 until recordTypes.length()) {
            try {
                ...
                when (recordType.lowercase()) {
                    ...
                    "heart_rate" -> permSet.add(Permission.of(DataTypes.HEART_RATE, AccessType.READ))
                    "blood_pressure" -> permSet.add(Permission.of(DataTypes.BLOOD_PRESSURE, AccessType.READ))
                    "body_composition" -> permSet.add(Permission.of(DataTypes.BODY_COMPOSITION, AccessType.READ))
...
//
//    /**
//     * 권한을 확인하고, 없으면 요청한 후 권한이 있을 경우 작업을 실행합니다.
//     *
//     * @param context 권한 요청을 위한 컨텍스트
//     * @param permissions 필요한 권한 세트
//     * @param activityId 권한 요청 식별자
//     * @param onGranted 권한이 허용되었을 때 실행할 작업
//     * @return 권한 처리 결과와 activityId 쌍
//     */
//    suspend fun checkPermissionsAndRun(
//        context: Context,
//        activityId: Int,
